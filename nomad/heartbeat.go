...
func (h *nodeHeartbeater) disconnectState(id string) (bool, bool) {
	...
	// and if so, whether it has pending reconnects. If one alloc isn't yet expired
	// it still has pending reconnects.
	canDisconnect := false
	hasPendingReconnects := false
	for _, alloc := range allocs {
		if !canDisconnect && alloc.DisconnectTimeout(now).After(now) {
			canDisconnect = true
		}
		if !hasPendingReconnects && alloc.DesiredStatus == structs.AllocDesiredStatusRun && !alloc.Expired(now) {
			hasPendingReconnects = true
			break
		}
		if canDisconnect && hasPendingReconnects {
			break
	...
	return canDisconnect, hasPendingReconnects
}
